...
// int func_from_vector(const std::string& s, counter<N>, const
// std::vector<int>& v, Args... args)
// {
// 	if(N < v.size())
...
// auto func_from_vector(const std::string& s, counter<end>, const
// std::vector<int>& v, Args... args)
// {
// 	if(end < v.size())
...
int main() {
  ...
  // TODO: The expected output of the with param tests should be taken from
  // ncurses or tparm Observation1: Negative values don't work with cuf, cup,
  // cuu, hpa, il, indn, rin caps(maybe -ve values don't work at all)
  // Observation2: Goto_col(hpa) cap only needs one arg, but %i increments
  // params if atleast 2 params are present, i think %i should check existence
  // individually for each of the first two args [FIXED].
  std::map<std::string,
           std::tuple<std::string, std::vector<valType>, std::string>>
      param_caps{
          ...
          {"cursor_right(cuf)_2", {"\E[%p1%dC", {-19}, "\E[-19C"}}, // Wrong
          ...
          {"set_bg_color(setab)_2",
           {"\E[%?%p1%{8}%<%t4%p1%d%e%p1%{16}%<%t10%p1%{8}%-%d%e48;5;%p1%d%;m",
            {10},
            "\E[102m"}},

          {"set_bg_color(setab)_3",
           {"\E[%?%p1%{8}%<%t4%p1%d%e%p1%{16}%<%t10%p1%{8}%-%d%e48;5;%p1%d%;m",
            {20},
            "\E[48;5;20m"}},
  ...
  std::cout << std::string(40, '*') << '\n'
            << '[' << passed_no_param_tests << '/' << total_no_param_tests
            << "] NO_PARAM TESTS PASSED\n"
            << '[' << passed_param_tests << '/' << total_param_tests
            << "] PARAM TESTS PASSED\n"
            << std::string(40, '*') << '\n';
  ...
  // this works wierd std::cout << "going to insert some characters..\E[7@did
  // anything change?\n"; // this doesn't work std::cout << "we will go to
  // column 54 \E[196GDid we get there ?\n"; // this depends on size of terminal
  // window std::cout << "some chars will be deleted now \E[10X Did chars
  // delete?\n"; // this doesn't work std::cout << "some lines will be deleted
  // now\n\E[6MDid lines delete?\n"; // this doesn't work std::cout << "some
  // chars will be deleted now\E[10PDid any chars delete?\n"; // this doesn't
  // work std::cout << "the cursor will move up now \E[7A Did it move up?\n";
  // std::cout << "the cursor will move up now \E[-10A Did it move up?\n"; //
  // this doesn't work std::cout << "can you check if the cursor moves to (55,
  // 80) now? \E[55;80H Did it move ?\n"; // this doesn't work std::cout << "can
  // you check if the cursor moves to (-55, -80) now? \E[-55;-80H Did it move
  // ?\n"; std::cout << "hi there, the cursor will move to the right now \E[20C
  // Did it move ?\n"; std::cout << "hi there, the cursor will move to the right
  // now(maybe left)\E[-4CDid it move ?\n"; auto test(){
  ...
  // std::cout << "check this init string:" << no_param_caps["init_string"] <<
  // "cool ?\n"; std::cout << "check blank mode" << no_param_caps["blank_mode"]
  // << "this is cool, check with input on\n"; std::cout << "a line will be
  // inserted now.." << no_param_caps["insert_line"] << " did you see it ?\n";
  // // mysterious working std::cout << "the cursor will tab now.." <<
  // no_param_caps["tab"] << " did it ?\n"; // doesn't work std::cout << "the
  // cursor will go home now.." << no_param_caps["cursor_home"] << " did it
  // ?\n"; std::cout << "the screen will flash now.. " <<
  // no_param_caps["flash_screeen"] << " did it ?\n"; // doesn't work std::cout
  // << "hello something " << no_param_caps["clear_to_eol"] << " worked ?\n"; //
  // doesn't work std::cout << "will this clear to end of screen? \E[J cleared
  // ?\n"; // doesn't work std::cout << " hello something delete line cap:
  // aaa\E[M hello otherthing\n"; std::cout << "bel cap:" <<
  // no_param_caps["bel"] << "hear something ?\n"; std::cout << "cr cap:" <<
  // no_param_caps["cr"] << "helo\n"; std::cout << "back_tab cap:" <<
  // no_param_caps["back_tab"] << '\n'; std::cout << "this will not print" <<
  // no_param_caps["clear"] << "this will print\n"; std::cout << "this will make
  // the cursor invisible" << no_param_caps["make_invisible"] << '\n'; int n =
  // 10; std::cin >> n; std::cout << "this will make the cursor visible" <<
  // no_param_caps["make_visible"] << '\n';
